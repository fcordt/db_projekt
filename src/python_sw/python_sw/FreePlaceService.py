...
class FreePlaceService:
    ...
    def is_ticket_free(self, ticket_nr: int):
        with self._connection.cursor() as cursor:
            for row in cursor.execute("SELECT COUNT(*) FROM EINZELTICKET_MIT_SPR E WHERE E.TICKETNUMMER = :tnr AND COALESCE(E.RESERVIERDATUM, TO_DATE('01011991', 'ddmmyyyy')) > sysdate - interval '10' minute",
                                      [ticket_nr]):
                return row[0] == 0
            ...
            for platz in cursor.execute("""
                                            ...
                                            AND NOT EXISTS (
                                                ...
                                                AND ET.ABFAHRT_BAHNHOF_NAME IN (
                                                    ...
                                                    AND SA.ABFAHRTSZEIT < (
                                                        SELECT COALESCE(SA1.ABFAHRTSZEIT, SA1.ANKUNFTSZEIT)
                                                        FROM STRECKENABSCHNITT SA1
                                                        WHERE SA1.FAHRPLAN_NR = :fpnr
                                                        AND COALESCE(SA1.ABFAHRT_BAHNHOF_NAME, SA1.ANKUNFT_BAHNHOF_NAME) = :ankunft_bahnhof
                                                    )
                                                )
                                                AND ET.ANKUNFT_BAHNHOF_NAME IN (
                                                    ...
                                                    WHERE SA.FAHRPLAN_NR = :fpnr
                                                    AND SA.ANKUNFTSZEIT > (
                                                        SELECT COALESCE(SA1.ANKUNFTSZEIT, SA1.ABFAHRTSZEIT)
                                                        FROM STRECKENABSCHNITT SA1
                                                        WHERE SA1.FAHRPLAN_NR = :fpnr
                                                        AND COALESCE(SA1.ANKUNFT_BAHNHOF_NAME, SA1.ABFAHRT_BAHNHOF_NAME) = :abfahrt_bahnhof
                                                    )
                                                )
